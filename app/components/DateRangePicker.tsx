...
const DateRangePicker: React.FC<DateRangePickerProps> = ({ onChange }) => {
  ...
  const [calendarMonths, setCalendarMonths] = useState({
    left: startOfMonth(subMonths(today, 1)),
    right: startOfMonth(today),
  });
  ...
  const updateCalendarMonths = useCallback(
    (start: Date, end: Date) => {
  ...
  const renderCalendar = useCallback(
    (month: Date, isLeftCalendar: boolean) => {
        ...
        <div className="w-64">
          ...
          <div className="flex justify-between items-center mb-2">
            {isLeftCalendar && (
              <button
                className="p-1"
                onClick={() => handleMonthNavigation("left")}
              >
            ...
            {!isLeftCalendar && (
              <button
                className="p-1"
                onClick={() => handleMonthNavigation("right")}
              >
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {days.map((day) => {
              const isSelected =
                (tempDateRange.start && isSameDay(day, tempDateRange.start)) ||
              ...
              const isInRange =
                ...
                !isSameDay(day, tempDateRange.start) &&
                !isSameDay(day, tempDateRange.end);
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-full ${
                    isSelected
                      ...
                      : isInRange
                        ? "bg-[rgb(69,69,69)] text-white"
                        : isDisabled
                          ? "text-[rgb(112,112,112)] cursor-not-allowed"
                          : "hover:bg-[#27272C] hover:text-white"
                  }`}
                  disabled={isDisabled}
  ...
  return (
    <Popover
      ...
      shouldCloseOnInteractOutside={(element) =>
        !element.closest(".popover-content")
      }
